"""mDNS announcers so a real Fronius Wattpilot discovers this emulated device.

Fronius devices find each other on the local network via two mechanisms: a
plain `_http._tcp.local.` Bonjour service (handled by `FroniusMDNSAnnouncer`
via Home Assistant's shared zeroconf instance), and a proprietary
`_Fronius-SE-Inverter._tcp.local.` / `_Fronius-SE-SmartMeter._tcp.local.`
announcement carrying a Fronius-specific "FSED-DID" TXT record and chunked
DeviceMeta JSON (handled by `RawMDNSAnnouncer`, which builds DNS packets by
hand since the proprietary labels exceed zeroconf's 15-byte TXT-string
assumptions). Without both, the Wattpilot's PV-surplus pairing step will not
find this integration on the network.

Ported from and credit to https://github.com/l2smith2/fronius-virtual-inverter
(MIT licensed), which reverse-engineered this from real Fronius device
traffic and validated it against Wattpilot firmware 42.5.
"""
...
_FLAGS_RESPONSE = 0x8400  # QR=1, Opcode=0, AA=1
...
class RawMDNSAnnouncer:
    """Announces the Fronius-SE service types via raw UDP multicast (IPv4 + IPv6).
    ...
    def _make_txt(self) -> bytes:
        ...
        chunk = 250  # value bytes per TXT string; "00=" prefix keeps total within 255
    ...
    async def async_start(self) -> None:
        ...
        _LOGGER.info(
            ...
            self._name, local_ip, self._port,
            FRONIUS_SE_INVERTER_TYPE, FRONIUS_SE_METER_TYPE,
        )
...
class FroniusMDNSAnnouncer:
    """Announces the emulated device via mDNS using HA's shared Zeroconf instance."""
    ...
    async def async_start(self, hass: HomeAssistant) -> None:
        ...
        _LOGGER.info(
            ...
            self._name, local_ip, self._port, MDNS_HTTP_TYPE,
        )
